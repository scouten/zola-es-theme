...
| `gain_m`, `loss_m` | int? | On `hike` and `walk` legs: metres climbed and descended, counting only changes of at least 5 m from the last altitude counted, since GPS altitude wobbles by a few metres. The caption shows them on a line of their own: "↑ 120 m / 390 ft · ↓ 5 m / 20 ft". |
