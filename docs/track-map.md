...
Modes: `drive`, `walk`, `hike`, `bike`, `run`, `bus`, `train`, `cable`, `boat`, `fly`, `stop`. A leg without a mode shows its distance only. A file with `photos` absent still works: photos snap to the nearest track point in page order.
...
Durations appear only on `fly` and `boat` legs. No clock times are shown or shipped.
